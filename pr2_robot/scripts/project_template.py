...
def get_dropbox_side(object_group):
...
def find_dropbox_param(dropbox_list_param, name):
...
def pr2_mover(object_list):
    ...
    test_scene_num.data = scene_number_param

    # TODO: Rotate PR2 in place to capture side tables for the collision map

    if (len(object_list_param) != len(object_list)):
        return

    dict_list = []
    # Loop through the pick list
    for index in range(len(object_list_param)):
        object_name = String()
        arm_name = String()
        pick_pose = Pose()
        place_pose = Pose()

        object_param = object_list_param[index]
        object_name.data = object_param['name']
        object = find_object(object_list, object_name.data)
        if object != None:
            ...
            print 'Object "{}" not found.'.format(object_name.data)
 
        # Assign the arm to be used for pick_place
        arm_name.data = get_dropbox_side(object_param['group'])

        # Create 'place_pose' for the object
        dropbox_param = find_dropbox_param(dropbox_list_param, arm_name.data)
        assert(dropbox_param != None)
        dropbox_pos = dropbox_param['position']
...
if __name__ == '__main__':
    ...
    scene_number_param = rospy.get_param('/scene_number')
